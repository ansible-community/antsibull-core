...
The current major version is 2.x.y. Development for 2.x.y occurs on the `main` branch. 1.x.y is now End of Life and was developed on the `stable-1` branch. 2.x.y mainly differs from 1.x.y by dropping support for Python 3.6, 3.7, and 3.8. It deprecates several compatibility functions for older Python versions that are no longer needed; see the changelog for details.
